...
// 轮函数
void SHA256::Round(uint32_t a, uint32_t b, uint32_t c, uint32_t &d, uint32_t e,
...
std::string SHA256::ShaFile(string path) {
  ...
  */
  ...
  int l =
      length + ((length % 64 >= 56) ? (128 - length % 64) : (64 - length % 64));
...
void SHA256::Transform(uint32_t *s, const uint8_t *chunk, size_t blocks) {
  while (blocks--) {
    ...
    for (int j = 0; j < 48; j++) {
      Round(a, b, c, d, e, f, g, h, K[16 + j],
            w[j % 16] += sigma1(w[(14 + j) % 16]) + w[(9 + j) % 16] +
                         sigma0(w[(1 + j) % 16]));
      i = a;
      a = h;
      h = g;
