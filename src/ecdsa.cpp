...
ECDSA::ECDSA() {
  try {
    if ((err = mp_init(&Hm)) != MP_OKAY) {
      throw("Error initializing the Hm. %s", mp_error_to_string(err));
    }
    ...
    if ((err = mp_init(&rA)) != MP_OKAY) {
      throw("Error initializing the rA. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&sA)) != MP_OKAY) {
      throw("Error initializing the sA. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&rB)) != MP_OKAY) {
      throw("Error initializing the rB. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&sB)) != MP_OKAY) {
      throw("Error initializing the sB. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&K)) != MP_OKAY) {
      throw("Error initializing the K. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&GX)) != MP_OKAY) {
      throw("Error initializing the GX. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&GY)) != MP_OKAY) {
      throw("Error initializing the GY. %s", mp_error_to_string(err));
    }
    if ((err = mp_init_set(&n, 1)) != MP_OKAY) {
      throw("Error initializing the n. %s", mp_error_to_string(err));
    }
    if ((err = mp_init_set(&h, 1)) != MP_OKAY) {
      throw("Error initializing the h. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&d)) != MP_OKAY) {
      throw("Error initializing the d. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&k)) != MP_OKAY) {
      throw("Error initializing the k. %s", mp_error_to_string(err));
    }
    ...
    if ((err = mp_init_set(&B, 7)) != MP_OKAY) {
      throw("Error initializing the B. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&PX)) != MP_OKAY) {
      throw("Error initializing the PX. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&PY)) != MP_OKAY) {
      throw("Error initializing the PY. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&X1)) != MP_OKAY) {
      throw("Error initializing the X1. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&Y1)) != MP_OKAY) {
      throw("Error initializing the Y1. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&u1X)) != MP_OKAY) {
      throw("Error initializing the u1X. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&u1Y)) != MP_OKAY) {
      throw("Error initializing the u1Y. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&u2X)) != MP_OKAY) {
      throw("Error initializing the u2X. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&u2Y)) != MP_OKAY) {
      throw("Error initializing the u2Y. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&X2)) != MP_OKAY) {
      throw("Error initializing the X2. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&Y2)) != MP_OKAY) {
      throw("Error initializing the Y2. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&v)) != MP_OKAY) {
      throw("Error initializing the v. %s", mp_error_to_string(err));
    }
    ...
    if ((err = mp_init(&k1)) != MP_OKAY) {
      throw("Error initializing the k1. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&s1)) != MP_OKAY) {
      throw("Error initializing the s1. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&n1)) != MP_OKAY) {
      throw("Error initializing the n1. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&temp)) != MP_OKAY) {
      throw("Error initializing the temp. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&u1)) != MP_OKAY) {
      throw("Error initializing the u1. %s", mp_error_to_string(err));
    }
    if ((err = mp_init(&u2)) != MP_OKAY) {
      throw("Error initializing the u2. %s", mp_error_to_string(err));
    }
    n.alloc = 512;
...
void ECDSA::printECDSA(const std::string &sh) {
  ...
  do {
    mp_rand(&rA, lon);
  } while (mp_cmp(&rA, &p_1) != -1 && mp_cmp_d(&rA, 1) != 1);
  do {
    mp_rand(&rB, lon);
  } while (mp_cmp(&rB, &p_1) != -1 && mp_cmp_d(&rB, 1) != 1);
