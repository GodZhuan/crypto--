...
// PC1 置换表
const unsigned int DES::PC1_Table[PC_1_SIZE] = {
    57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42, 34, 26, 18, 10, 2,  59, 51, 43,
    35, 27, 19, 11, 3,  60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7,  62, 54,
    46, 38, 30, 22, 14, 6,  61, 53, 45, 37, 29, 21, 13, 5,  28, 20, 12, 4};

// 移动表
const unsigned int DES::Move_Table[SUBKEY_NUM] = {1, 1, 2, 2, 2, 2, 2, 2,
                                                  1, 2, 2, 2, 2, 2, 2, 1};
// PC2 置换表
...
// IP 置换表
const unsigned int DES::IP_Table[BIT_STR_SIZE] = {
...
// 扩展置换表
const unsigned int DES::Expand_Table[EXPAND_SIZE] = {
    32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,  8,  9,  10, 11,
    12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21,
    22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1};

// P 置换表
const unsigned int DES::Permute_Table[BIT_STR_SIZE / 2] = {
    16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23, 26, 5,  18, 31, 10,
    2,  8, 24, 14, 32, 27, 3,  9,  19, 13, 30, 6,  22, 11, 4,  25};

// IP^-1 置换表
const unsigned int DES::IP_1_Table[BIT_STR_SIZE] = {
...
// S 盒表
const unsigned int DES::SBox_Table[KEY_SZIE][4][16] = {
    // S1
...
std::string DES::Decrypt(const std::string &cipher, const std::string &key) {
  std::string result;
  if (cipher.empty() || key.empty() || cipher.size() % 8 || !CreateSubKey(key))
    return result;
...
bool DES::EncryptBlock(std::string &block) {
  ...
  for (size_t i = 0; i < SUBKEY_NUM; ++i) {
    ...
    bool subKey[SUBKEY_LENGHT];
    std::copy(subKeys[i], subKeys[i] + SUBKEY_LENGHT, subKey);
...
void DES::PC1_Transform(const bool bitStr[BIT_STR_SIZE],
                        bool PC1bitStr[PC_1_SIZE]) {
  for (size_t i = 0; i < PC_1_SIZE; ++i)
    PC1bitStr[i] = bitStr[PC1_Table[i]];
}

void DES::PC2_Transform(const bool PC1bitStr[PC_1_SIZE],
                        volatile bool subKey[SUBKEY_LENGHT]) {
  std::string tmpStr;
...
void DES::SBox_Transform(bool eBitStr[EXPAND_SIZE]) {
  for (size_t i = 0; i < KEY_SZIE; ++i) {
    ...
    size_t column = (eBitStr[j + 1] << 3) + (eBitStr[j + 2] << 2) +
                    (eBitStr[j + 3] << 1) + eBitStr[j + 4];
...
void DES::XOR(bool strFirst[EXPAND_SIZE], bool strSecond[EXPAND_SIZE],
              size_t num) {
...
bool DES::LeftCycle(bool str[PC_1_SIZE], size_t step) {
  // 左循环函数，用于将 DES 初始密钥（56 位）扩展为 48 位子密钥
  // 参数：
  //   str: 指向 56 位初始密钥的指针
  //   step: 循环步长，即子密钥的长度
  bool temp[PC_1_SIZE]; // 临时数组，用于存储中间结果

  // 复制 str 数组的前 step 个元素到 temp 数组
  memcpy(temp, str, step);
  // 复制 str 数组的后 28 - step 个元素到 temp 数组的 step 个元素位置
  memcpy(temp + step, str + 28, step);

  // 复制 str 数组的中间 28 - step 个元素到 str 数组的前 step 个元素位置
  memcpy(str, str + step, 28 - step);
  // 复制 temp 数组的 step 个元素到 str 数组的中间 28 - step 个元素位置
  memcpy(str + 28 - step, temp, step);

  // 复制 str 数组的后 28 - step 个元素到 temp 数组的前 step 个元素位置
  memcpy(str + 28, str + 28 + step, 28 - step);

  // 复制 str 数组的中间 28 - step 个元素到 temp 数组的 step 个元素位置
  memcpy(temp + step, str + 28 + (28 - step), 28 - step);

  // 复制 temp 数组的 step 个元素到 str 数组的后 28 - step 个元素位置
